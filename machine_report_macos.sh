#!/bin/bash
# TR-100 Machine Report (macOS Compatible)
...
get_ip_addr() {
    ...
    if command -v ifconfig &> /dev/null; then
        ...
        ipv4_address=$(ifconfig | awk '
            /^[a-z]/ {iface=$1}
            iface != "lo0:" && iface !~ /^docker/ && /inet / && !/127\.0\.0\.1/ && !found_ipv4 {found_ipv4=1; print $2}')

        # If IPv4 address not available, try IPv6 using ifconfig
        if [ -z "$ipv4_address" ]; then
            ipv6_address=$(ifconfig | awk '
                /^[a-z]/ {iface=$1}
                iface != "lo0:" && iface !~ /^docker/ && /inet6 / && !/::1/ && !found_ipv6 {found_ipv6=1; print $2}')
        fi
    fi
...
# Get DNS from scutil
net_dns_ip=($(scutil --dns | grep 'nameserver\[[0-9]*\]' | awk '{print $3}' | sort -u))

# CPU Information (macOS)
cpu_model="$(sysctl -n machdep.cpu.brand_string | awk '{print $1 " " $2 " " $3 " " $4}')"
cpu_hypervisor="Bare Metal"

# Check if running in a VM
if sysctl -n machdep.cpu.features | grep -q "VMX\|SVM"; then
    vm_check=$(system_profiler SPHardwareDataType | grep "Model Name" | grep -i "virtual\|vmware\|parallels")
    if [ -n "$vm_check" ]; then
...
cpu_sockets="1"
cpu_freq="$(sysctl -n hw.cpufrequency 2>/dev/null | awk '{ printf "%.2f", $1 / 1000000000 }')"
if [ -z "$cpu_freq" ]; then
    cpu_freq="$(sysctl -n hw.cpufrequency_max 2>/dev/null | awk '{ printf "%.2f", $1 / 1000000000 }')"
fi
if [ -z "$cpu_freq" ]; then
    cpu_freq="N/A"
fi
...
# Get memory pressure and calculate used memory
...
mem_used_bytes=$(((pages_wired + pages_active + pages_compressed) * page_size))
mem_used_kb=$((mem_used_bytes / 1024))

mem_percent=$(awk -v used="$mem_used_kb" -v total="$mem_total_kb" 'BEGIN { printf "%.2f", (used / total) * 100 }')
mem_total_gb=$(echo "$mem_total_kb" | awk '{ printf "%.2f", $1 / (1024 * 1024) }')
mem_used_gb=$(echo "$mem_used_kb" | awk '{ printf "%.2f", $1 / (1024 * 1024) }')

# Disk Information (macOS)
root_partition="/"
disk_info=$(df -g "$root_partition" | awk 'NR==2 {print $2, $3}')
root_total_gb=$(echo "$disk_info" | awk '{printf "%.2f", $1}')
root_used_gb=$(echo "$disk_info" | awk '{printf "%.2f", $2}')
...
if [ $uptime_days -gt 0 ]; then
    sys_uptime="${uptime_days}d "
fi
if [ $uptime_hours -gt 0 ]; then
    sys_uptime="${sys_uptime}${uptime_hours}h "
fi
if [ $uptime_minutes -gt 0 ]; then
    sys_uptime="${sys_uptime}${uptime_minutes}m"
fi
sys_uptime=$(echo "$sys_uptime" | sed 's/ $//')
...
disk_bar_graph=$(bar_graph "$root_used_gb" "$root_total_gb")
